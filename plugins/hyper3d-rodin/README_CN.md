...
1. 正式发布后，在 ZCode 插件管理器中安装并启用 **Hyper3D Rodin**。
   本地测试本次贡献时，打开「发现」标签页，点击 **+**，添加包含
   `marketplace.json` 和 `plugins/hyper3d-rodin/` 的本地市场仓库目录，
   再从该市场安装并启用插件。
2. 通过 ZCode 的 MCP 认证入口在浏览器中登录 Hyper3D，核对请求授权的客户端，
   选择计费工作空间并授权。不要把 API Key、密码、Cookie 或会话 Token
   放入聊天或本插件配置。
...
Skill 指导文字或图片生成、限时等待进度和结果获取。参考图支持 1–5 张，每张不超过
...
- **服务与网络：** 对话模型由 ZCode 提供，Rodin/BANG 在线服务由 Hyper3D 提供。
  ...
  签名 URL 中的存储/CDN 域名。这些 URL 是临时地址，不应记录或分享。
...
1. 从本地市场安装，确认插件与 Skill 加载。市场图标的官方 CDN 地址在发布后才可用。
2. 完成 OAuth 登录，测试 Token 刷新与重新连接，并对测试账号拥有的生成任务执行
   一次授权后的状态查询。
3. 在明确要求执行会消耗额度的测试后，生成一个模型，等待完成并打开永久结果页面；
   另外验证参考图片 PUT 上传、BANG 拆件和用户要求的文件下载。
4. 确认连接/状态检查不会提交生成任务，错误不会触发收费任务自动重提。

本次贡献尚未完成 ZCode 安装、OAuth/刷新和端到端生成验证。协议发现成功或仓库校验
通过不能代替上述客户端检查。
